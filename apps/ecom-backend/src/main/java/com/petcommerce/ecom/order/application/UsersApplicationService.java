package com.petcommerce.ecom.order.application;

import com.petcommerce.ecom.order.domain.user.aggregate.User;
import com.petcommerce.ecom.order.domain.user.repository.UserRepository;
import com.petcommerce.ecom.order.domain.user.service.UserReader;
import com.petcommerce.ecom.order.domain.user.service.UserSynchronizer;
...
import com.petcommerce.ecom.order.infrastructure.secondary.service.kinde.KindeService;
import com.petcommerce.ecom.shared.authentication.application.AuthenticatedUser;
...
public class UsersApplicationService {
  ...
  public User getAuthenticatedUser(Jwt jwtToken) {
    return userReader.getByEmail(new UserEmail(AuthenticatedUser.username().get())).orElseThrow();
  }
